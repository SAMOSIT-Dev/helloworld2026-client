<script lang="ts">
    ...
    let {
        ...
        required,
        icon,
        // 1. เพิ่ม Prop ใหม่เพื่อเช็คว่าเป็น TextArea หรือไม่
        isTextArea = false 
    } = $props<{
        label?: string;
        placeholder?: string;
...
<div class="flex flex-col w-full">
    ...
    <div
        class={cn(
            'group flex w-full border-2 rounded-2xl md:rounded-[25px] overflow-hidden transition-all duration-200 min-w-0',
            isTextArea ? 'items-start min-h-[120px] md:min-h-[150px]' : 'items-center min-h-[52px] md:h-[65px]',
            error
    ...
    >
        {#if icon}
            <div class={cn('flex px-3 md:px-4 relative shrink-0', isTextArea ? 'pt-4' : 'items-center')}>
        ...
        {#if isTextArea}
            <textarea
                ...
                {onblur}
                {placeholder}
                class="flex-1 min-w-0 w-full px-3 md:px-4 py-3 bg-transparent focus:outline-none placeholder:opacity-30 text-sm md:text-base text-white resize-none"
            ></textarea>
        {:else}
            <input
                {id}
        ...
        {#if error}
            <div
                ...
                class={cn('flex pr-3 md:pr-5 text-red-500 shrink-0', isTextArea ? 'pt-4' : 'items-center')}
            >
                <svg
