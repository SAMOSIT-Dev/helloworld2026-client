...
<Card class={cn('w-48 h-72', className)}>
	<div class="w-full h-full flex items-center justify-center flex-col gap-1">
		...
		<div class="flex flex-col gap-1">
			<div class="text-center text-white text-xl font-semibold font-['Anantason'] line-clamp-1">
				{file.title}
			</div>

			<div>
				<ClockIcon color={role?.color} class="inline-block w-4 h-4 mr-1" />
				<span class="text-white text-sm font-semibold font-['Anantason']">{file.timeLabel}</span>
			</div>

			<div>
				<DatabaseIcon color={role?.color} class="inline-block w-4 h-4 mr-1" />
				<span class="text-white text-sm font-semibold font-['Anantason']">{file.sizeLabel}</span>
			</div>
		</div>
		...
		<button
			...
			class="cursor-pointer w-36 h-8 bg-neutral-600 rounded-md border border-neutral-400 py-2.5 px-3.5 flex items-center justify-between mt-2 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:border-neutral-400"
		>
			<span class="text-white text-sm font-semibold font-['Anantason']">Download</span>
			{#if isDownloading}
