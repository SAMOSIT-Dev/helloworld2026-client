...
<Card class={cn('w-41.5 h-58 md:w-48 md:h-72', className)}>
	<div class="w-full h-full flex items-center justify-center flex-col gap-1">
		...
		<div class="flex flex-col gap-0 md:gap-1">
			<div class="text-center text-white text-md md:text-xl font-semibold line-clamp-1">
				{file.title}
			</div>
		...
		<button
			...
			class="cursor-pointer w-32 h-7 md:w-36 md:h-8 bg-neutral-600 rounded-md border border-neutral-400 py-2 md:py-2.5 px-3 md:px-3.5 flex items-center justify-between mt-1 md:mt-2 disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:*:text-white/30 disabled:border-neutral-400"
		>
			<span class="text-white text-xs md:text-sm font-semibold">Download</span>
			{#if isDownloading}
